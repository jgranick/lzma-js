...
namespace LZMA
{
	...
	LZMA_buffer* LZMA::Compress(const LZMA_buffer &buffer) {

		SizeT inputBufferSize = buffer.length;
		Byte* inputBufferData = buffer.data;
        ...
        LZMA_buffer* result = malloc(sizeof(struct LZMA_buffer));
        result->length = outputBufferSize + propsSize + 8;
	...
	LZMA_buffer* LZMA::Decompress(const LZMA_buffer &buffer)
    {
		SizeT inputBufferSize = buffer.length;
		Byte* inputBufferData = buffer.data;
        ...
        result->data = malloc(uncompressedLength);
